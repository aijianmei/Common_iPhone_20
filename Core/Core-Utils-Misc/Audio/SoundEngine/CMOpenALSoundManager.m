...
- (void) playBackgroundMusicByUrl:(NSURL *)fileUrl forcePlay:(BOOL)forcePlay
{
	if(forcePlay)	//if we want to kill other audio sources, like the iPod...
	{
		[backgroundAudio stop]; //if there's audio already playing...
	...
	self.currentBackgroundAudioFile = [fileUrl absoluteString];
	[player setNumberOfLoops:-1]; //loop forever
	player.volume = backgroundMusicVolume;
	[player play];
...
{	
	...
	[self playSoundWithSoundFile:soundFile];
}

- (void) playSoundWithSoundFile:(NSString*)soundFile
{
	//get sound key
...
- (void) playSoundWithSoundFileUrl:(NSURL *)soundFileUrl
{
    CMOpenALSound *sound = [soundDictionary objectForKey:soundFileUrl];
	
	if(!sound)
	{
		//create a new sound
		sound = [[CMOpenALSound alloc] initWithSoundFileUrl:soundFileUrl doesLoop:NO]; //this will return nil on failure
		
		if(!sound) //error
			return;
		
		[soundDictionary setObject:sound forKey:soundFileUrl];
		[sound release];
	}
