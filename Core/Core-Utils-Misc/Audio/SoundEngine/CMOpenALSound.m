...
-(BOOL) loadSoundFile:(NSString *)file
             doesLoop:(BOOL)loops;
- (BOOL) loadSoundFileWithUrl:(NSURL *)fileUrl
                     doesLoop:(BOOL)loops;
...
- (id) initWithSoundFileUrl:(NSURL *)fileUrl doesLoop:(BOOL)loops
{
    self = [super init];
	if (self != nil)
	{
		if(![self loadSoundFileWithUrl:fileUrl doesLoop:loops])
		{
			debug(@"Failed to load the sound file: %@...", fileUrl);
			[self release];
			return nil;
		}
		self.sourceFileName = [NSString stringWithContentsOfURL:fileUrl encoding:NSStringEncodingConversionAllowLossy error:nil];
...
#pragma mark -
#pragma mark loading
- (BOOL) loadSoundFileWithUrl:(NSURL *)fileUrl doesLoop:(BOOL)loops
{
    ALenum  format;
	ALsizei size;
	ALsizei freq;
    
    CFURLRef fileURL = (CFURLRef)[fileUrl retain];
	
	if (!fileURL)
	{
		debug(@"file: %@ not found.", fileUrl);
		return NO;
	}
...
//	ALsizei freq;
	...
	return [self loadSoundFileWithUrl:[NSURL fileURLWithPath:path] doesLoop:loops];
}
...
- (BOOL) play
{
	if([self isPlaying]) //see if the base source is busy...
	{
        ...
        ALint value;
        alGetSourcei(tmpSourceID,AL_SOURCE_STATE,&value);
        //    NSLog(@"%x",value);
        if (value != AL_PLAYING)
        {
