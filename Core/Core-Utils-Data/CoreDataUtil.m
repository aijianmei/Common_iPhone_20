...
- (NSString*)descriptionForFetchRequest:(NSFetchRequest*)fq
{
    return [NSString stringWithFormat:@"%@ %@", [fq entityName], [[fq predicate] description]];
}
...
- (NSArray*)execute:(NSString*)fetchRequestName
{
    ...
    if (fq == nil){
        PPDebug(@"<execute> fetch request (%@) fail, cannot create fetch request", fetchRequestName);
        return nil;
    }
	...
	if (error == nil){
		PPDebug(@"<execute> fetch request (%@) success, total %d record found", fetchRequestName, [objects count]);
//		for (NSObject* item in objects){
//			PPDebug(@"[Debug] result object (%@)", [item description]);
//		}
	}
	else {
		PPDebug(@"<execute> fetch request (%@), error=%@", fetchRequestName, [error localizedDescription]);
	}		
...
{    
	...
	NSArray* objects = [self.managedObjectContext executeFetchRequest:fq error:&error];
	if (error == nil){
		PPDebug(@"<execute> fetch request (%@) success, total %d record", fetchRequestName, [objects count]);
//		for (NSObject* item in objects){
//			PPDebug(@"[Debug] result object (%@)", [item description]);
//		}
	}
...
- (NSObject*)execute:(NSString*)fetchRequestName forKey:(NSString*)primaryKey value:(NSObject*)value 
{
	...
	NSError* error = nil;    
//    PPDebug(@"<execute> fetch request (%@)", [self descriptionForFetchRequest:fq]);
	NSArray* objects = [self.managedObjectContext executeFetchRequest:fq error:&error];
	if (error == nil){
		PPDebug(@"<execute> fetch request (%@) success", [self descriptionForFetchRequest:fq]);
	}
	else {
		PPDebug(@"<execute> fetch request (%@), error=%@", [self descriptionForFetchRequest:fq], [error localizedDescription]);
	}			
...
- (NSArray*)execute:(NSString*)fetchRequestName keyValues:(NSDictionary*)keyValues sortBy:(NSString*)sortKey ascending:(BOOL)ascending
{
    ...
    if (fq == nil){
        PPDebug(@"<execute> fetch request (%@) fail, key values = %@, cannot create fetch request", [keyValues description], fetchRequestName);
        return nil;
    }
	...
	NSArray* objects = [self.managedObjectContext executeFetchRequest:fq error:&error];
	if (error == nil){
		PPDebug(@"<execute> fetch request (%@) success, total %d record found", [self descriptionForFetchRequest:fq], [objects count]);
		//		for (NSObject* item in objects){
		//			PPDebug(@"[Debug] result object (%@)", [item description]);
		//		}
	}
...
- (NSArray*)execute:(NSString*)fetchRequestName forKey:(NSString*)key value:(NSObject*)value sortBy:(NSString*)sortKey ascending:(BOOL)ascending
{
    ...
    if (fq == nil){
        PPDebug(@"<execute> fetch request (%@) fail, cannot create fetch request", [self descriptionForFetchRequest:fq]);
        return nil;
    }
	...
	NSArray* objects = [self.managedObjectContext executeFetchRequest:fq error:&error];
	if (error == nil){
		PPDebug(@"<execute> fetch request (%@) success, total %d record found", [[fq predicate] description], [objects count]);
		//		for (NSObject* item in objects){
		//			PPDebug(@"[Debug] result object (%@)", [item description]);
		//		}
	}
	else {
		PPDebug(@"<execute> fetch request (%@), error=%@", [[fq predicate] description], [error localizedDescription]);
	}		
...
{
	PPDebug(@"<del> delete object (%@)", [[object entity] name]);
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
	...
	if (![fileManager fileExistsAtPath:storePath]) {
		...
		if (defaultStorePath) {
			if ([fileManager copyItemAtPath:defaultStorePath toPath:storePath error:NULL] == YES){
				PPDebug(@"Copy DB to %@ success!", storePath);
			}
			else {
	...
	PPDebug(@"addPersistentStoreWithType success!");
