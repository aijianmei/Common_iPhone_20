...
- (void)dealloc
{
    [_facebook setSessionDelegate:nil];
    PPRelease(_facebook);
    [super dealloc];
}
...
{
    PPDebug(@"<removeFromService> clear session delegate");
    [self.facebook setSessionDelegate:nil];
    [super removeFromService];
}
...
- (void)fbDidLogin
{
    ...
    if (self.action == SNS_ACTION_LOGIN || self.action == SNS_ACTION_LOGOUT){
...
- (NSMutableDictionary*)parseUserInfo:(NSDictionary*)origUserInfo
{
    ...
    // store user id here
    NSNumber* userId = [origUserInfo objectForKey:@"uid"];
    if (userId != nil){
        [self.service setUserId:[NSString stringWithInt:[userId intValue]]];
        [[PPSNSStorageService defaultService] storeAuthData:self.service];
    }
